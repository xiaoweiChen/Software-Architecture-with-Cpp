...
应该如何实现\texttt{Square}类的成员？如果想要遵循LSP，并避免让这些类的使用者感到意外，但这不可能。如果调用\texttt{setWidth}，正方形将不再是正方形。所以，这里只能停止使用正方形(使用前面的代码是无法表达的)，或修改高度，从而使正方形看起来不同于矩形。
...
因为\texttt{Project}现在是一个模板，必须在每次创建它时指定类型列表，或者提供一个类型别名。可以像这样使用:
