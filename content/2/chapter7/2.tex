...
如果正在使用CMake，并且没有绑定到特定的生成器(例如，CLion需要使用Code::Blocks生成器)，可以用更快的生成器替换默认的Make生成器。Ninja是一个很好的开始，它专门为减少构建时间而创建的，只需在调用CMake时传递\texttt{-G Ninja}就可以使用。
...
\begin{table}[H]
	\begin{tabular}{|l|l|}
		...
		-Wshadow              & \begin{tabular}[c]{@{}l@{}}A 所有关于覆盖了之前声明的警告。\end{tabular}                   \\ \hline
	\end{tabular}
\end{table}
