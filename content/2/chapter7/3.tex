...
src目录中，应该有另一个CMakeLists.txt文件，这次可能定义了一个或两个目标。让我们为前面提到的多米尼加博览会系统添加一个客户微服务的可执行文件:
...
相反，生成器表达式是实现相同目标的正确方法，因为它们将在稍后处理。让我们来看一个实际应用的例子。假设想为Release配置添加一个预处理器定义，可以这样写:
